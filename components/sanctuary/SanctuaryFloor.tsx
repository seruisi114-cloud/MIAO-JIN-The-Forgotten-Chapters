    ...
    <group>
      ...
      <group ref={mapRef} scale={initialMapScale}>
        {[
          ...
          <Line key={index} points={arcPoints(arc.radius, arc.start, arc.length)} color={sanctuaryPalette.champagneGold} lineWidth={0.52 - index * 0.08} transparent opacity={arc.opacity} />
        ))}
      </group>
    </group>
  );
