...
export function CreatorArchiveCore({ chapterPositions, activeIndex, skipIntro = false, onOpenCreatorNote }: CreatorArchiveCoreProps) {
  ...
  const archiveRingRefs = useRef<Array<THREE.Mesh | null>>([]);
  ...
  useFrame(({ clock }, delta) => {
    ...
    archiveRingRefs.current.forEach((ring, index) => {
      if (ring) ring.rotation.z += delta * speeds[index] * (hovered.current ? 2.1 : 0.7);
    });
  ...
  return (
    <group ref={rootRef} position={[0, 0.08, 0]} scale={skipIntro ? 1 : 0.001}>
      ...
      <group position={[0, 1.26, -0.08]}>
        <mesh position={[0, 0, -0.06]}>
          <circleGeometry args={[0.78, 72]} />
          <meshPhysicalMaterial color="#090f1b" roughness={0.42} metalness={0.36} clearcoat={0.24} transparent opacity={0.7} side={THREE.DoubleSide} />
        </mesh>
        {[0.9, 0.72, 0.54].map((radius, index) => (
          <mesh
            key={radius}
            ref={(node) => { archiveRingRefs.current[index] = node; }}
            rotation={[0, 0, index * 0.72]}
          >
            <torusGeometry args={[radius, index === 0 ? 0.012 : 0.007, 8, 112, index === 1 ? Math.PI * 1.44 : Math.PI * 1.72]} />
            <meshBasicMaterial color={index === 2 ? "#d8dce5" : "#bca16b"} transparent opacity={0.45 - index * 0.07} depthWrite={false} />
          </mesh>
        ))}
        {Array.from({ length: 20 }, (_, index) => {
          const angle = index / 20 * Math.PI * 2;
          return (
            <mesh key={index} position={[Math.cos(angle) * 0.99, Math.sin(angle) * 0.99, 0]} rotation={[0, 0, angle]}>
              <boxGeometry args={[index % 5 === 0 ? 0.115 : 0.048, 0.008, 0.008]} />
              <meshBasicMaterial color="#c6aa73" transparent opacity={index % 5 === 0 ? 0.56 : 0.26} depthWrite={false} />
            </mesh>
          );
        })}
      </group>
      ...
      <mesh ref={coreRef} position={[0, 1.26, 0.08]}>
      ...
      <mesh position={[0, 1.26, 0.04]} scale={3.2}>
      ...
      <mesh position={[0, 0.72, 0]}>
        <cylinderGeometry args={[0.13, 0.42, 1.55, 48, 1, true]} />
        <meshBasicMaterial color="#e0e6ef" transparent opacity={0.052} depthWrite={false} blending={THREE.AdditiveBlending} />
      </mesh>
      <pointLight ref={lightRef} position={[0, 1.28, 0.12]} color="#e7e7df" intensity={0.5} distance={5.6} decay={2.15} />
      ...
      <Sparkles count={42} scale={[2.4, 2.2, 2.2]} size={0.66} speed={isHovered ? 0.18 : 0.075} color="#c3a76e" opacity={isHovered ? 0.5 : 0.3} noise={0.35} />
    </group>
  );
}
